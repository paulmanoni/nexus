...
// startFrontendWatcher runs the project's Vite dev server. Vite is
// the only frontend engine: it spawns the user's `npm run dev`
// (Vite dev server + HMR), sniffs stdout for the "Local:" URL so
// the dashboard can auto-open against it, and dedups identical
// rebuild blocks. The nexus proxy (/__nexus,/graphql,/oauth,/ws →
// the Go app) is injected into vite.config by runDev via
// EnsureViteProxyForNexus, not wired here.
