...
func runReconcile(opts reconcileOptions, stdout, stderr io.Writer) error {
	...
	// Operator-authored sections (deployments, peers, overrides)
	// untouched. Overrides are re-applied AFTER the rewrite so they
	// patch the freshly-emitted code declarations rather than the
	// stale ones we just discarded.
	dm.Services = mergeServices(pm.Services)
	dm.Env = mergeEnv(pm.Env)
	dm.StartupTasks = mergeTasks(pm.StartupTasks)

	appliedOv, staleOv := dm.ApplyOverrides()
	if len(staleOv) > 0 {
		// Surface stale overrides as a hint, not a hard error — the
		// most common case is a service rename in source that the
		// operator hasn't propagated to the override yet. Better to
		// flag and continue than to fail the reconcile.
		fmt.Fprintf(stderr, "nexus reconcile: warning: %d override(s) target unknown declarations: %s\n",
			len(staleOv), strings.Join(staleOv, ", "))
	}
	...
	buf.WriteString("# Use the `overrides:` block at the bottom for per-deploy patches that\n")
	buf.WriteString("# survive reconciliation (env-name remappings, service-kind swaps, etc.).\n\n")
	...
	if appliedOv > 0 {
		fmt.Fprintf(stdout, "applied %d override(s) on top of code declarations\n", appliedOv)
	}
