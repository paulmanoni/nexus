...
//
// The directories are created with 0o755 so that a project shared
...
// DefaultRoot returns ~/.nexus/cache. Falls back to a temp dir when
// the home directory isn't resolvable (CI containers, etc.) — the
// cache still works, it just won't survive the process.
func DefaultRoot() string {
	home, err := os.UserHomeDir()
	if err != nil || home == "" {
		return filepath.Join(os.TempDir(), "nexus-cache")
	}
