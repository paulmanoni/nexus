...
//
// Filter is anchored to ".vue" extension. esbuild calls our
// OnLoad only when the path ends in .vue, so the filter doesn't
// need to be permissive.
//
// Behavior on compile errors: we surface them as esbuild
// Messages, which causes the build to fail loudly with the file
// + line + col + message. We do NOT emit any code when the
// compiler reports errors — better to fail than to feed esbuild
// half-compiled JS that produces a second cascade of errors.
