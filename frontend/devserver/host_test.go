...
// Host.
func fixture(t *testing.T) (*httptest.Server, *Host) {
	t.Helper()
	root := t.TempDir()
	...
	return httptest.NewServer(viteless.NewServer(h).Handler()), h
}
...
func TestHost_ServesAssetAsURLExport(t *testing.T) {
	ts, _ := fixture(t)
	defer ts.Close()
	code, body := get(t, ts.URL, "/src/logo.png")
	if code != 200 {
...
func TestHost_ServesCSSAsStyleModule(t *testing.T) {
	ts, _ := fixture(t)
	defer ts.Close()
	code, body := get(t, ts.URL, "/src/style.css")
	if code != 200 {
