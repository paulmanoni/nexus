...
type Secret struct {
	...
	// rules. Optional.
	RotateAlert string         `json:"rotateAlert,omitempty"`
...
type EnvValidation struct {
	...
	Min    *int     `json:"min,omitempty"`    // numeric values
	Max    *int     `json:"max,omitempty"`    // numeric values
	Length *Range   `json:"length,omitempty"` // string length
}
...
type Override struct {
	...
	EnvSpecs    map[string]EnvVarPatch    `json:"envSpecs,omitempty"`
	SecretSpecs map[string]SecretPatch    `json:"secretSpecs,omitempty"`
	FileSpecs   map[string]FilePatch      `json:"fileSpecs,omitempty"`
